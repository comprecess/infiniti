...
export const putSendEmail = async (
  id: number,
  viewType: string,
  info: PartialFieldsPostData,
): Promise<Response> => {
  if (!Number.isInteger(id) || id <= 0 || !viewType) {
    return {
      status: false,
  ...
  try {
    ...
    const apiPath = import.meta.env.VITE_CUSTOMERS_VIEW_ADD_NEW_ELEMENT
    ...
    const url = new URL(`${apiPath}${id}/view/${viewType}`, baseUrl).toString()
